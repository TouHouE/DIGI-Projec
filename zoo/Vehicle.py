...
class Vehicle:
    def __init__(self, plate_serial, parking_time, plate_image, parking=True, leave_time=None, owner=''):
        ...
        # try:
        #     self.plate_image_str = IC.img2base64(plate_image)
    ...
    def __str__(self):
        if self.parking:
            ...
            return f'{"=" * 15}\nOwner: {self.owner}\nAll Plate Serials: {self.plate_serial}\nDuration: {(self.leave_time - self.in_park_time).total_seconds()} seconds, Parking: {self.parking}'
...
def make_vehicle(plate_serials: dict, parking_time, plate_image, owner_map, top_n_accuracy=3, create_threshold=100) -> Vehicle:
    ...
    # this line for owner hasn't been recorded
    print(f'Oops! Look like {top_n_plate} hasn\'t been recorded yet!')
    return Vehicle(top_n_plate, parking_time, plate_image)